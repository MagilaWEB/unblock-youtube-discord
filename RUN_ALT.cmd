...
:check_Permissions
    ...
    if %errorLevel% == 0 (
        ...
        net stop "%SRVCNAME%"
        sc delete "%SRVCNAME%"
        sc create "%SRVCNAME%" binPath= "\"%~dp0bin\winws.exe\" %ARGS%" DisplayName= "DPI обход блокировки tcp-udp : %SRVCNAME%" start= auto
        sc description "%SRVCNAME%" "DPI программное обеспечение для обхода блокировки."
        sc start "%SRVCNAME%"
        
        net stop "%SRVCNAME2%"
        sc delete "%SRVCNAME2%"
        sc create "%SRVCNAME2%" binPath= "\"%~dp0bin\goodbyedpi.exe\" %ARGS2%" DisplayName= "DPI обход блокировки http : %SRVCNAME2%" start= auto
        sc description "%SRVCNAME2%" "Passive Deep Packet Inspection blocker and Active DPI circumvention utility"
        sc start "%SRVCNAME2%"

        schtasks /Create /F /TN winws2 /NP /RU "" /SC onstart /TR "\"%~dp0RUN_ALT.cmd\""
