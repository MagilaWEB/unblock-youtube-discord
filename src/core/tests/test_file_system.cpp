...
TEST_CASE("File reading lines", "[file][read]")
{
    ...
    f.forLine([&](auto str) { lines.push_back(str); return false; });
...
TEST_CASE("File forLine stops early", "[file][read]")
{
    ...
    std::vector<std::string> lines;
    f.forLine([&](auto str) { lines.push_back(str); return str == "b"; });
...
TEST_CASE("File forLine empty file", "[file][read]")
{
    ...
    int count = 0;
    f.forLine([&](auto) { count++; return false; });
    CHECK(count == 0);
}
...
TEST_CASE("forLineSection iterates section lines", "[file][section]")
{
    ...
    std::vector<std::string> lines;
    f.forLineSection("Network", [&](auto& str) { lines.push_back(str); return false; });
    REQUIRE(lines.size() == 2);
...
TEST_CASE("forLineSection caches after first call", "[file][section]")
{
    ...
    f.forLineSection("General", [&](auto& str) { first.push_back(str); return false; });

    std::vector<std::string> second;
    f.forLineSection("General", [&](auto& str) { second.push_back(str); return false; });
...
TEST_CASE("forLineSection missing section", "[file][section]")
{
    ...
    int count = 0;
    f.forLineSection("DoesNotExist", [&](auto&) { count++; return false; });
    CHECK(count == 0);
}
...
TEST_CASE("forLineParametersSection parses key=value", "[file][section]")
{
    ...
    f.forLineParametersSection("General", [&](auto k, auto v) { kv[k] = v; return false; });
    REQUIRE(kv.size() == 2);
...
TEST_CASE("File forLineSection with empty section", "[file][edge]")
{
    ...
    int count = 0;
    f.forLineSection("Empty", [&](auto&) { count++; return false; });
    CHECK(count == 0);
}
