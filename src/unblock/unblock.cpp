...
std::list<Service>& Unblock::getConflictingServices()
{
    ...
    Service::allService(
        [&](std::string name_service)-> void
        {
            ...
            constexpr static std::string_view services_conflict[]{
                "winws.exe", "winws2.exe", "goodbyedpi.exe", "ciadpi.exe"
            };
...
std::vector<std::string> Unblock::listVersionStrategy()
{
    ...
    std::ranges::sort(strategy_dirs, [](const std::string& left, const std::string& right)
    {
        return Core::get().isVersionNewer(left, right);
    });
